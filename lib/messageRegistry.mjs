...
import { buildTrainingCampActiveMessage } from "../messages/trainingCampActive.mjs";
...
// Every message type, in the order it occurs during the league year. Each
// entry is one of:
//   - autoTrigger(league) -> truthy trigger data | null   (checkable from the API alone)
//   - externalCheckHint: string                            (needs a human/agent lookup; no autoTrigger)
// build(league, trigger) returns { subject, body, needsManualInput }.
//
...
export const MESSAGE_TYPES = [
  ...
  },
  {
    ...
    autoTrigger: null,
    externalCheckHint:
      "Not derivable from Sleeper's API. Web search for this year's official NFL training camp report dates " +
      "(all 32 teams) and find the earliest one. If that date has passed and is on/after today, this message is due.",
